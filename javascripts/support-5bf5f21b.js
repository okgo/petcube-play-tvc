...
(function() {
  var YouTubeVideo, showSupportPopup;
  ...
  showSupportPopup = function(e) {
    ...
    $this = $(this);
    $popup = $('#support-popup');
    return $popup.fadeIn('fast', function() {
      return $(window).trigger('modalShow');
    }).find('.close').on('click', function(e) {
      e.preventDefault();
  ...
  $(document).on('click', '#support-popup-open', showSupportPopup);

  $(document).on('support-popup', showSupportPopup);

  $(function() {
    var video, videoId;
    $('#slide-to-support-email').on('click', function(e) {
      var bannerHeight, menuHeight, scrollOffset, scrollTop;
      e.preventDefault();
      bannerHeight = $('.banner').height();
      menuHeight = $('.header-menu').height();
      scrollOffset = $('.support-form-section').offset().top;
      scrollTop = scrollOffset - bannerHeight - menuHeight;
      return $('html,body').animate({
        scrollTop: scrollTop
      }, 'slow');
    });
    if ($('#support-video').length > 0) {
      videoId = $('#support-video').data('id');
      video = new YouTubeVideo('support-video', videoId);
      video.init(800, 450);
    }
    $('#was-helpful-yes').on('click', function() {
      var thanks;
      thanks = $('.helpful-thanks').clone();
      $('.was-helpful').hide();
      return $('.was-helpful-container .wrap').html(thanks);
    });
    $('#was-helpful-no').on('click', function() {
      var form;
      form = $('.helpful-form').clone();
      $('.was-helpful').hide();
      $('.was-helpful-container .wrap').html(form);
      return $('.cancel-helpful-form').on('click', function(e) {
        e.preventDefault();
        console.log('click');
        $('.was-helpful').show();
        return $(this).parents('.helpful-form').remove();
      });
    });
    $('#support-search__field').focusin(function() {
      return $('.support-search__searchbox').addClass('visible');
    });
    $('#support-search__field').focusout(function() {
      return $('.support-search__searchbox').removeClass('visible');
    });
    $('.support-form').on('submit', function(e) {
      e.preventDefault();
      $('.notification-container').addClass('visible');
      return setTimeout((function() {
        return $('.notification-container').removeClass('visible');
      }), 3000);
    });
    return $('.js-close-notification').on('click', function(e) {
      e.preventDefault();
      return $('.notification-container').removeClass('visible');
    });
  });
